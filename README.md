...
[![Version](https://img.shields.io/badge/version-1.2.5-emerald.svg?style=for-the-badge)](package.json)
...
### 🪟 Windows (.exe)
Download and run the 64-bit installer `xTerminal Setup 1.2.5.exe`.

### 🍎 macOS (.dmg)
Download `xTerminal-1.2.5-arm64.dmg` (Apple Silicon) or `xTerminal-1.2.5-x64.dmg` (Intel).
...
- **Direct Sideload APK**: Download `xTerminal-1.2.5.apk`, enable *"Install Unknown Apps"* in Android Settings, and install.
- **Google Play Store App Bundle (.aab)**: Download `xTerminal-1.2.5-playstore.aab` from Releases, or build locally via `.\build.ps1 -Target aab`.
